...
max_price = 20  # Enter your Max Price your willing to pay and include taxes.
webpage_refresh_timer = 4  # Default 4 seconds. If slow internet and the page isn't fully loading, increase this.
test_mode = False  # Set False for testing. If set to True, it will place order and checkout product.
...
def login_attempt(driver):
    ...
    try:
        username_field = driver.find_element("css selector", '#ap_email')
    
        username_field.send_keys(username)
        driver_wait(driver, 'css', '#continue')
        password_field = driver.find_element("css selector", '#ap_password')
        password_field.send_keys(password)
        ##AmazonCaptcha('captcha.jpg').solve()
...
def attempt_purchase(driver, index=-1):
    ...
    try:
        buy_box = format_price(driver.find_element("css selector", '.a-price'))
        if buy_box <= max_price:
            driver.find_element("css selector", "#buy-now-button")  # Attempt to find buy now button.
    ...
    # try:
    #     asking_to_login = driver.find_element("css selector", '#ap_password').is_displayed()
    #     if asking_to_login:
    ...
    if not test_mode:
        ...
        iframe = driver.find_element("css selector", '#turbo-checkout-iframe')
        if iframe:
            driver.switch_to.frame(iframe)
            driver_wait(driver, 'css', '#turbo-checkout-pyo-button', True)  # Wait for inner iframe content to load
        ...
        # time.sleep(.5)
        driver_wait(driver, 'css', 'a-button-input')  # Click place order button on modal
        notify_and_exit()
        return True
